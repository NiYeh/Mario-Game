...
public class Player3 : MonoBehaviour
{
    ...
    private void OnCollisionEnter2D(Collision2D collision)
    {
        ...
        // 如果碰到的是敵人
        else if (collision.gameObject.tag == "enemy1" || collision.gameObject.tag == "virus")
        {
            // UI 生命减1
            ...
            if (UIManager.Life.LifeNum <= 0)
            {
                ...
                // 播放第一個背景音效 並去掉背景音樂
                AudioManager3.Instance.Play(0, true);
                // 3.5秒後觸發遊戲結束
                Invoke("GameOver", 3.5f);
            }
            else
            {
                AudioManager3.Instance.Play(4);
                GetComponent<Rigidbody2D>().AddForce(Vector2.up * -100);
            }
        }
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        {
            // 弹开
            GetComponent<Rigidbody2D>().AddForce(Vector2.up * 750);
        }
        ...
        {
            // 暂停背景音樂
            AudioManager3.Instance.MusicStop();
