...
public class Player2 : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        // 讓音樂管理器查背景音樂
        AudioManager2.Instance.CheckBG();
    }
    ...
    private void OnCollisionEnter2D(Collision2D collision)
    {
        ...
        else if (collision.gameObject.tag == "enemy1")
        {
            ...
            if (UIManager.Life.LifeNum <= 0)
            {
                ...
                // 播放第一個背景音效 並去掉背景音樂
                AudioManager2.Instance.Play(0, true);
                // 3.5秒後觸發遊戲結束
                Invoke("GameOver", 3.5f);
            }
            else
            {
                AudioManager2.Instance.Play(4);
                GetComponent<Rigidbody2D>().AddForce(Vector2.up * -100);
            }
        }
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        {
            // 暂停背景音樂
            AudioManager2.Instance.MusicStop();
